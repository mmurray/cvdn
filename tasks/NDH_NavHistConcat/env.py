...
class R2RBatch():
    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
                 path_type='planner_path', history='target'):
        ...
        for item in datasets:  
            ...
            if history == 'target' or len(item['dialog_history']) == 0:  # Have to use target only if no dialog history.
                tar = item['target']
    ...
    def _get_obs(self):
        ...
        for i,(feature,state) in enumerate(self.env.getStates()):
            ...
            if 'nav_hist_encoding' in item:
                obs[-1]['nav_hist_encoding'] = item['nav_hist_encoding']
        return obs

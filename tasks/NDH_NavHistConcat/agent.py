...
class Seq2SeqAgent(BaseAgent):
    ...
    def _sort_nav_history(self, obs):
        ''' Extract instructions from a list of observations and sort by descending
            sequence length (to enable PyTorch packing). '''

        seq_tensor = np.array([ob['nav_hist_encoding'] for ob in obs])
        seq_lengths = np.argmax(seq_tensor == "", axis=1)
        seq_lengths[seq_lengths == 0] = seq_tensor.shape[1] # Full length

        encoded_seq_tensor = np.zeros((len(obs), 12, 2048))
        mask_seq = np.zeros((len(obs), 12))
        for i, ob in enumerate(obs):
            for j, s in enumerate(encoded_seq_tensor[i]):
                if seq_tensor[i][j] == "":
                    encoded_seq_tensor[i][j] = [0] * 2048
                    mask_seq[i][j] = 1
                    continue
                pieces = seq_tensor[i][j].split("_")
                encoded_seq_tensor[i][j] = self.env.env.features[ob['scan'] + '_' + pieces[0]][int(pieces[1]),:]

        encoded_seq_tensor = torch.from_numpy(encoded_seq_tensor)
        seq_lengths = torch.from_numpy(seq_lengths)


        # Sort sequences by lengths
        seq_lengths, perm_idx = seq_lengths.sort(0, True)
        sorted_tensor = encoded_seq_tensor[perm_idx]
        mask = torch.from_numpy(mask_seq)

        perm_idx = list(perm_idx)
        reverse_perm_idx = [0] * len(perm_idx)
        for i,v in enumerate(perm_idx):
            reverse_perm_idx[v] = i

        return Variable(sorted_tensor, requires_grad=False).float().cuda(), \
               mask.byte().cuda(), \
               list(seq_lengths), perm_idx, reverse_perm_idx
    ...
    def rollout(self):
        ...
        nav_hist_seq, nav_hist_mask, nav_hist_lengths, nav_perm_idx, nav_reverse_perm_idx = self._sort_nav_history(obs)
        combined_mask = torch.cat((seq_mask, nav_hist_mask), 1)
        ...
        ctx,h_t,c_t = self.encoder(seq, seq_lengths, nav_hist_seq, nav_hist_lengths, nav_reverse_perm_idx, perm_idx)
        ...
        for t in range(self.episode_len):
            ...
            h_t,c_t,alpha,logit = self.decoder(a_t.view(-1, 1), f_t, h_t, c_t, ctx, combined_mask)
